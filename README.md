...
## WSL preparation for Open5GS

* Enable WSL2 and virtualization: in a PowerShell terminal run ```wsl --install```, reboot, then set default version to 2 with ```wsl --set-default-version 2```.
* Run ```wsl --list --online``` to see a list of all available distros and versions.
* Install a specific Ubuntu distro using a NAME from the output above ```wsl --install Ubuntu-22.04```. For alternatives way refer to official [documentation](https://documentation.ubuntu.com/wsl/stable/howto/install-ubuntu-wsl2/).
* On PowerShell run wsl and update and upgrade.

  ```
  sudo apt update
  sudo apt upgrade -y```
* Install basic tools you’ll use later with Open5GS:

```
sudo apt install -y wget gnupg net-tools iproute2
```
Once this is in place, you can proceed with [Open5GS](https://open5gs.org/open5gs/docs/guide/01-quickstart/) installation directly following the Open5GS quickstart for Ubuntu, inside this WSL Ubuntu 22.04 environment.
